...
class Topology:
    ...
    def __process_key(self, number: int = None) -> int:
        keys = [edge[2] for edge in self._graph.edges(keys=True)]
    ...
    def get_link(self, key: int):
        for edge in self._graph.edges(keys=True):
            if edge[2] == key:
                return edge
    ...
    def connect_devices(self, device_id1: str | int, port1: str, device_id2: str | int, port2: str,
        ...
        self._graph.add_edge(self[device_id1], self[device_id2], key=key, bandwidth=link_bandwidth)
